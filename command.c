...
#define UAP_SSID  "SGxTL-M-130614"
...
void mxchipScanCompleteHandler(UwtPara_str *pApList)
{
    ...
    if (cmd != 0)
        emsp_send_cmdpkt(cmd, apnum, respbuf, scanlen, 0);
}
...
//  sprintf(pbase->wifi_ssid, "MXCHIP_%02X%02X%02X", mac[3], mac[4], mac[5]);
   	sprintf(pextra->uap_ssid, UAP_SSID, sizeof(UAP_SSID));
...
//  pextra->uap_key = UAP_KEY;
    ...
    sprintf(pextra->my_device_name, "EMW_3280 (%02X%02X%02X)", mac[3], mac[4], mac[5]);
...
void emsp_cmd_do(u32 cmd, u32 len, u8 *indata, u32 *outlen, u32 *presult)
{
    switch (cmd)
    {
...
{
    u8 *retdata = NULL;
    u32 result, retlen = 0, *pval=(u32*)respbuf, endtime;
    base_config_t *pbase = &running_config.base;
    extra_config_t *pextra = &running_config.extra;
    ...
    switch (cmd)
    {
        ...
        case EMSP_CMD_GET_CONFIG:           // get current system configuration and return them to HOST
            result = 1;
            retdata = (u8 *)&running_config.base;
...
void mdns_begin(int iface)
{
    ...
    sprintf(txt_attr, "Vendor=MXCHIP.Version=%s.", tmpstr);
    if (pbase->use_udp == 1)
    {
        if (pbase->connect_mode == 0)
        {
    ...
    wlan_get_mac_address(mac);
    sprintf(tmpstr, "EMW3280_%02X%02X%02X.local", mac[3], mac[4], mac[5]);
